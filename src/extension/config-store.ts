...
export function loadRuntimeConfigState(packageConfigDir: string, userConfigDir = packageConfigDir): LoadedConfigState {
	...
	try {
		...
		return {
			...
			examplePath,
			asGateState() {
				return { blocked: this.blocked, diagnostics: this.diagnostics, message: this.message, configPath: this.configPath, examplePath: this.examplePath };
			},
		};
	} catch (error) {
...
export function createRuntimeConfigStore(packageConfigDir: string, userConfigDir = packageConfigDir): RuntimeConfigStore {
	...
	return {
		...
		getGateState: () => gate,
		reloadConfig() {
			current = loadRuntimeConfigState(packageConfigDir, userConfigDir);
			assignGate(current, gate);
		},
	};
}
